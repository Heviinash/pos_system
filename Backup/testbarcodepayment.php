...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Only loop if barcode and quantity are present
    if (!empty($barcodes) && !empty($quantities)) {
...
<div class="container">
    <form method="POST" action="paymenttesting.php" id="salesForm">
        <div class="left-panel">
            ...
            <div id="scanner" style="margin-bottom: 20px;">
                ...
                <input type="text" id="barcode" placeholder="Scan barcode here..." autofocus style="width: 100%; padding: 10px; margin-bottom: 10px; border: 1px solid #ccc; border-radius: 5px;" oninput="processBarcode()">
    ...
    <script>
        ...
        // Function to handle barcode scanning from the external barcode scanner
        let lastDetectedBarcode = ''; // Store the last detected barcode
        let isProcessing = false; // Flag to prevent multiple processing

        function processBarcode() {
            const barcode = document.getElementById('barcode').value; // Get the barcode value from input field

            // Prevent processing the same barcode multiple times and check if it's a valid barcode
            if (barcode !== lastDetectedBarcode && !isProcessing && barcode.length > 0) {
                isProcessing = true; // Set processing flag
                lastDetectedBarcode = barcode; // Update the last detected barcode

                // Set the barcode input value (you can use this value for other actions if needed)
                document.getElementById('barcode').value = barcode;

                // Trigger an action, like adding the product
                document.getElementById('addProduct').click(); // This can be a custom function to add the product
        ...
        // Optional: Focus on the barcode input field when the page loads
        window.onload = () => {
            document.getElementById('barcode').focus(); // Focus the barcode input field
        };
