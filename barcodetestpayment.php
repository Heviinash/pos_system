...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_SESSION['transaction_id'])) {
        ...
        generatePDFReceipt($conn, $_SESSION['transaction_id']);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($stmt->execute()) {
        ...
        // Step 1: Get the current running balance
        // Step 1: Get the current running balance (only the first row in the table)
        $balanceQuery = "SELECT balance FROM running_balance LIMIT 1";
        $balanceResult = $conn->query($balanceQuery);
        $currentBalance = 0;

        if ($balanceResult->num_rows > 0) {
            // If there is an existing balance, fetch it
            $row = $balanceResult->fetch_assoc();
            $currentBalance = $row['balance'];
        }

        // Step 2: Add the grand total to the current balance
        $newBalance = $currentBalance + $grand_total;

        // Step 3: Update the balance in the running_balance table
        $updateBalanceQuery = "UPDATE running_balance SET balance = ? WHERE 1 LIMIT 1";
        $stmtBalance = $conn->prepare($updateBalanceQuery);
        $stmtBalance->bind_param("d", $newBalance);

        if ($stmtBalance->execute()) {
            echo "Running balance updated successfully!";
        } else {
            echo "Error updating running balance: " . $stmtBalance->error;
        }
...
<div style="display: flex; justify-content: center; margin: 20px auto; clear: both;">
        <button style="width: 150px; padding: 8px 12px; background-color: #28a745; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 14px; margin: 0 10px;" 
                onclick="window.location.href='mainpage.php'">
            Go to Main Page
        </button>
        <button style="width: 150px; padding: 8px 12px; background-color: #28a745; color: white; border: none; border-radius: 5px; cursor: pointer; font-size: 14px; margin: 0 10px;" 
                onclick="window.location.href='barcodetestpayment.php'">
            Next Transaction
        </button>
</div>
<div class="container">
    <form method="POST" action="barcodetestpayment.php" id="salesForm">
            ...
            <table id="productTable" style="width: 100%; border-collapse: collapse; margin-bottom: 20px; font-family: Arial, sans-serif; background-color: #31a3c8;">
                ...
                <tfoot>
                    <tr>
                        ...
                        <td id="subtotalCell" style="padding: 12px; text-align: right; font-weight: bold;">0.00</td>
                    </tr>
                </tfoot>
    ...
    <script>
        ...
        let barcodeTimeout; // Timeout reference to handle delayed input

        document.getElementById('barcode').addEventListener('input', function () {
            const barcodeField = this;
            clearTimeout(barcodeTimeout); // Clear any previous timeout
            ...
            barcodeTimeout = setTimeout(async () => {
                ...
                if (barcode) {
                    try {
                        ...
                        newRow.innerHTML = `
                            <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">${barcode}</td>
                            <td style="padding: 8px; border: 1px solid #ddd; text-align: center;">${productName}</td>
                            ...
                            <td style="padding: 8px; border: 1px solid #ddd; text-align: right;">${productPrice}</td> 
                            <td style="padding: 8px; border: 1px solid #ddd; text-align: right;" class="total-price">${totalPrice}</td>
                        `;
                        ...
                        hiddenQuantityInput.setAttribute('data-barcode', barcode);
            ...
            }, 500); // Wait for 500 milliseconds after typing stops
        });
        ...
        document.getElementById('productRows').addEventListener('input', function (e) {
            if (e.target.classList.contains('editable-quantity')) {
                const quantityCell = e.target;
                const newQuantity = parseInt(quantityCell.textContent.trim()) || 0;
                const productPrice = parseFloat(quantityCell.getAttribute('data-price')) || 0;
                const barcode = quantityCell.getAttribute('data-barcode');

                // Update total price for the row
                const totalPriceCell = quantityCell.parentElement.querySelector('.total-price');
                ...
                const hiddenQuantityInput = document.querySelector(`input[name="quantity[]"][data-barcode="${barcode}"]`);
        ...
        function updateGrandTotal(subtotal) {
            ...
            const fixedTaxRate = 6; // Fixed tax rate of 6%
            
            // Calculate discount
            const discountedTotal = subtotal - (subtotal * (discount / 100));
            
            // Calculate tax on the discounted total using the fixed tax rate
            const taxAmount = discountedTotal * (fixedTaxRate / 100);
            
            // Calculate grand total (discounted total + tax amount)
