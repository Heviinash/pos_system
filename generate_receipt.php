...
function generatePDFReceipt($conn, $transaction_id) {
    ...
    // Initialize the customer info string
    $customer_info = '';

    // Check if customer name is not empty before adding it
    if (!empty($customer_name)) {
        $pdf->SetFont('Helvetica', 'B', 14);
      //  $pdf->MultiCell(0, 10, 'Customer Information', 0, 'L', 0, 1);
        $pdf->SetFont('Helvetica', '', 12);

        $customer_info .= 'Customer Name: ' . htmlspecialchars($customer_name) . "\n";
    }

    // Check if IC number is not empty before adding it
    if (!empty($icnumber)) {
        $customer_info .= 'IC Number: ' . htmlspecialchars($icnumber) . "\n";
    }
    ...
    $pdf->SetFont('Helvetica', '', 12);
    $pdf->MultiCell(0, 10, $customer_info, 0, 'L', 0, 1);
}
    ...
    $html .= '<thead><tr><th>Barcode</th><th>Product Name</th><th>Qty</th><th>Total Price (RM)</th></tr></thead><tbody>';
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // Add the total price of the current row to the running sum
    ...
    // Add the total price row below the table
    $html .= '<p><strong>Total Price: RM ' . number_format($total_price_sum, 2) . '</strong></p>';
