...
$font_query = "SELECT font_family, font_size FROM invoicepdf_settings LIMIT 1";
...
$query = "SELECT sender_info_position FROM invoicepdf_settings LIMIT 1"; // You can adjust the query if you need more conditions
...
$watermark_query = "SELECT watermark_text FROM invoicepdf_settings LIMIT 1";
...
$pdf->Ln(15);
$pdf->Cell(0, 15, 'INVOICE', 0, 1, 'C'); // Centered title
...
$invoice_terms = isset($_SESSION['invoice_terms']) ? $_SESSION['invoice_terms'] : '1. Payment Terms: Payment is due upon receipt of the invoice unless otherwise agreed in writing.
2. Late Payments: Any overdue amounts may incur a late fee of 1.5% per month on the outstanding
balance.
3. Refund Policy: All sales are final. Refunds will only be issued for defective products or services,
and requests must be made within 14 days of purchase.
4. Delivery: Delivery times are estimates and may vary. We are not responsible for delays caused by
third parties.
5. Liability: Our liability is limited to the purchase price of the product or service. We are not liable for
any indirect or consequential damages.';

$pdf->MultiCell(0, 8, $invoice_terms, 0, 'L'); // Adjust line height for better spacing
...
$pdf_filename = "invoice_{$customer_name}_{$timestamp}.pdf"; 
